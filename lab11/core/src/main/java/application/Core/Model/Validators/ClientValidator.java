...
public class ClientValidator implements Validator<Client> {
    ...
    private static Validation<Client> correctAuthor = SimpleValidation.from((client) -> client.getFirstName().length() >= 4, "application.Client first name must contain at least 3 characters!");

    private static Validation<Client> correctTitle = SimpleValidation.from((client) -> client.getLastName().length() >= 4, "application.Client last name must contain at least 3 characters!");
